    ...
    def observe(self,
        ...
        for r in range(self.n_rows):
            ...
            for c in range(self.n_cols):
                ...
                w = 1                            #init weight
                x,y = self.get_coordinate(r,c)   #xy coordinates
                
                #updating weights and PDF
                for i in range(4):
                    
                    posT = self.belief[r][c]     #posterior
                    posX = landmarks[i][0]       #posterior x
                    posY = landmarks[i][1]       #posterior y   
                    
                    sq = pow((posX-x),2) + pow((posY-y),2)
                    dist = math.sqrt(sq)         #distance
                    probabilityDF = normal_pdf(dist, App.SENSOR_NOISE, observed_distances[i]) #PDF
                    w *= probabilityDF           #updated weight
                    
                self.belief[r][c] = posT* w
                
        self.belief = self.normalize(self.belief)  #normalizing belief
    ...
    def timeUpdate(self):
        ...
        for r in range(self.n_rows):
            ...
            for c in range(self.n_cols):
                ...
                actionList = self.transition_model(r, c)    #action list
                probPosT = self.belief[r][c]                #posterior probability
                
                for a in actionList:
                    
                    probTran = a[0]                         #transition probability
                    newR,newC = a[1]                        #next row and column
                    self.belief[newR][newC] += probPosT* probTran
...
class ParticleFilter(Inference):
    ...
    NUM_PARTICLES = 500  #increased paricled from 200 to 500
    ...
    def observe(self,
        ...
        """
        # Please finish the code below
        w=[]                                            #weight intialization
        
        for n in self.particles:
            
            r,c = n                                     #row and column of each particle
            x,y = self.get_coordinate(r,c)              #converting to xy coords
            prod = 1                                    #product of probabilities 
            ...
            for i in range(4):
                ...
                sq = pow((posX-x),2) + pow((posY-y),2)
                dist = math.sqrt(sq)                    #distance
                prod *= normal_pdf(dist, App.SENSOR_NOISE, observed_distances[i])  #updating product
                
            w.append(prod)                              #appending weight dsitribution
            
        prevN = self.particles[:]                       #list of previous particles
        
        for n in range(self.NUM_PARTICLES):
            
            newN = (random.choices(prevN, weights=w, k=1))[0]
            self.particles[n] = newN
    ...
    def timeUpdate(self):
        ...
        """
        # Please finish the code below
        for l in range(len(self.particles)): 
            
            actionList = self.transition_model(self.particles[l][0],self.particles[l][1])
            w = [0]* len(actionList)
            ...
            nextCell = random.choices(actionList, weights=w, k=1) 
            self.particles[l] = nextCell[0][1]
